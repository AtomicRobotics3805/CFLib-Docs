...
# CommandGroup
The `CommandGroup` class represents a command that contains and manages other sub-commands. This is not generally used by the end user, but is instead a utility to provide common functionality for the [`SequentialCommandGroup`](CommandGroup/SequentialCommandGroup) and [`ParallelCommandGroup`](CommandGroup/ParallelCommandGroup) classes.
